...
export default function FilterGroup(props) {
  ...
  const onClick = (name) => {
    ...
    let select = props.filterGroup.select;
    props.toggleFilter(filterGroupName, select, name);
  };
  ...
  const getButtonsForOptions = () => {
    return options.map((option, i) => {
      ...
      return (
        <button key={i} className={className} data-group-name={lowerCaseGroupName} onClick={() => onClick(option.name)}>
