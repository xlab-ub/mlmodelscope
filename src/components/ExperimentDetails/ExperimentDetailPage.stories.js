...
const MockContainer = (props) => {
  ...
  const failedTrials = props.failedTrials ?? [];
  const inputs = trials
    .filter(
      (trial, idx, arr) =>
        arr.findIndex((t) => t.inputs[0] === trial.inputs[0]) === idx
    )
    .map((tr) => tr.inputs[0]);
  ...
  const makeExperiment = () => ({
    id: "1234567489",
    trials: trials.filter((trial) => trial.inputs[0] === selectedInput),
  });
  const mockFn = () => {};
  ...
  return (
    <>
      <ExperimentDetailPage
        ...
        onDeleteTrial={() =>
          setModalType(ExperimentDetailModalTypes.confirmDeleteModel)
        }
        selectedInput={selectedInput}
        showAddInputModal={() =>
          setModalType(ExperimentDetailModalTypes.addInput)
        }
        showDeleteInputModal={() =>
          setModalType(ExperimentDetailModalTypes.confirmDeleteInput)
        }
        trialIsDeleting={false}
        trialToDelete={null}
        updateInput={(input) => setSelectedInput(input)}
        task={props.task ?? Task.image_classification}
...
ImageClassification.args = {
  trials: [TestImageClassificationResult, TestImageClassificationResult],
};
...
ImageClassificationMultipleInputs.storyName =
  "Image Classification (Multiple Inputs)";
...
ImageEnhancement.args = {
  trials: [TestImageEnhancementData, TestImageEnhancementData],
};
...
Text.args = {
  trials: [TestTextOutput, TestTextOutput],
  task: Task.text,
};

export const TextThreeTrials = Template.bind({});
...
export const TrialFailedTwoTrials = Template.bind({});
TrialFailedTwoTrials.args = {
  trials: [TestObjectDetectionResult, TestObjectDetectionResult],
  failedTrials: [0, 1],
};

export const TrialFailedOneTrial = Template.bind({});
TrialFailedOneTrial.args = {
  trials: [TestObjectDetectionResult],
  failedTrials: [0],
};
